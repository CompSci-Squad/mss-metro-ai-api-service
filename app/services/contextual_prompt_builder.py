...
class ContextualPromptBuilder:
    ...
    async def build_prompt(self, project_id: str, rag_context: dict | None = None, 
                          prompt_strategy: str = "confidence_aware") -> str:
    ...
    async def _get_previous_analysis(self, project_id: str) -> dict | None:
        try:
            analyses = list(ConstructionAnalysisModel.project_id_index.query(
                project_id, scan_index_forward=False, limit=1))
    ...
    def _parse_element(self, elem) -> dict:
        return elem if isinstance(elem, dict) else {
